...
@test expected_value(p -> 1, prior1) ≈ 1
@test (prior1.a ≈ 9.2) & (prior1.b ≈ 13.8)
@test mean(prior1) ≈ .4
prior_sd = expected_value(p -> (p - mean(prior1))^2, prior1) |> sqrt
@test prior_sd ≈ .1
pmf = predictive_pmf.(0:10, 10, prior1)
@test sum(pmf) ≈ 1

prior2 = condition(prior1; low = .3)
@test minimum(prior2.pivots) > .3
@test expected_value(p -> 1, prior2) ≈ 1
